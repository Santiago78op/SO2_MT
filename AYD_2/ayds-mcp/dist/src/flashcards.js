 ...
 */
import { CARPETAS, ErrorHerramienta, clave, leerTexto, listarArchivos, nombreDeNota, paraMostrar, parsearFrontmatter, rutaRelativa, sugerir, } from "./boveda.js";
/** Lista los mazos que existen, con su tema del frontmatter. */
export function listarMazos() {
 ...
 */
export function extraerFlashcards(contenido) {
 ...
 */
export function obtenerFlashcards(tema, cantidad) {
    ...
    // Pasada 3: contiene.
    elegido ??= mazos.find((m) => clave(m.archivo).includes(pedido) || clave(m.tema).includes(pedido));
    if (!elegido) {
        throw new ErrorHerramienta(`No hay flashcards para el tema "${tema}".`, mazos.map((m) => `${m.archivo}  (tema: ${m.tema})`));
    }
    const todas = extraerFlashcards(leerTexto(elegido.ruta));
    if (todas.length === 0) {
        throw new ErrorHerramienta(`El archivo ${elegido.ruta} existe pero no tiene tarjetas con formato "pregunta::respuesta".`, sugerir(tema, mazos.map((m) => m.archivo)));
    }
