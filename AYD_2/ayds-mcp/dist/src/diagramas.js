...
import { CARPETAS, ErrorHerramienta, clave, leerTexto, listarArchivos, nombreDeNota, normalizar, paraMostrar, rutaRelativa, sugerir, } from "./boveda.js";
// ---------------------------------------------------------------------------
 ...
 */
export function extraerBloquesMermaid(contenido) {
 ...
 */
export function obtenerDiagrama(nombre) {
    ...
    if (exacto)
        return conFuente(exacto);
    // --- Forma 2: archivo, con o sin extension ---
    const archivo = inventario.find((d) => d.origen === "archivo" &&
        (clave(d.id) === clave(pedido) ||
            clave(nombreDeNota(d.id)) === clave(pedido)));
    if (archivo)
        return conFuente(archivo);
    // --- Forma 3: nombre de nota ---
    const deLaNota = inventario.filter((d) => d.origen === "bloque-mermaid" && clave(d.nota ?? "") === clave(pedido));
    if (deLaNota.length === 1 && deLaNota[0]) {
        return conFuente(deLaNota[0]);
    }
    if (deLaNota.length > 1) {
        throw new ErrorHerramienta(`La nota "${pedido}" tiene ${deLaNota.length} diagramas. Pedi uno por su id exacto.`, deLaNota.map((d) => `${d.id}  (${d.tipo})`));
    }
    // No se encontro de ninguna forma.
    throw new ErrorHerramienta(`No encontre el diagrama "${nombre}". Usa listar_diagramas para ver los ids disponibles.`, sugerir(nombre, inventario.map((d) => d.id)));
}
/**
...
function conFuente(diagrama) {
    ...
    if (!bloque) {
        ...
        throw new ErrorHerramienta(`El diagrama "${diagrama.id}" ya no esta en ${diagrama.ruta}. ` +
 ...
 */
//# sourceMappingURL=diagramas.js.map