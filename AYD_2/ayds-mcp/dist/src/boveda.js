...
export class ErrorHerramienta extends Error {
    sugerencias;
    constructor(message, sugerencias = []) {
        super(message);
...
export function inicializarBoveda() {
    ...
    if (!crudo || crudo.trim() === "") {
        throw new Error("Falta la variable de entorno VAULT_PATH.\n" +
 ...
 */
export function rutaSegura(relativa) {
    const base = raiz();
    ...
    if (relativa.startsWith("/") || /^[a-zA-Z]:/.test(relativa)) {
        throw new ErrorHerramienta(`Ruta rechazada: se esperaba una ruta relativa a la boveda, no una absoluta ("${relativa}").`);
    }
    ...
    if (candidata !== base && !candidata.startsWith(base + sep)) {
        throw new ErrorHerramienta(`Ruta rechazada: "${relativa}" queda fuera de la boveda. ` +
    ...
    if (existsSync(candidata)) {
        ...
        if (real !== base && !real.startsWith(base + sep)) {
            throw new ErrorHerramienta(`Ruta rechazada: "${relativa}" es un enlace que apunta fuera de la boveda.`);
        }
        return real;
    }
    return candidata;
 ...
 */
export function rutaDeEscrituraProgreso() {
...
export function leerTexto(relativa) {
 ...
 */
export function listarArchivos(subcarpeta, extensiones) {
    ...
    return readdirSync(ruta)
        ...
        if (nombre.startsWith("."))
            return false; // .gitkeep, .DS_Store del Mac
 ...
 */
export function resolverArchivo(subcarpeta, nombrePedido, extensiones = [".md"]) {
    ...
    throw new ErrorHerramienta(`No encontre "${nombrePedido}" en ${subcarpeta}/.`, sugerir(nombrePedido, disponibles));
}
/**
 ...
 * en menos letras.
 */
export function sugerir(pedido, disponibles, maximo = 5) {
...
export function nombreDeNota(archivo) {
...
//# sourceMappingURL=boveda.js.map