    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Error interno en ${nombre}: ${detalle}\n` +
...
server.registerTool("listar_temas", {
    ...
    description: "Lista todas las notas de la boveda de Analisis y Diseno de Sistemas con su tema, " +
...
}, async () => ejecutar("listar_temas", () => {
    const notas = listarTemas();
    if (notas.length === 0)
        return "La carpeta 01-Notas/ esta vacia.";
...
// RF-02 — leer_nota
// ---------------------------------------------------------------------------
server.registerTool("leer_nota", {
    title: "Leer una nota completa",
    description: "Devuelve el contenido COMPLETO de una nota de 01-Notas/, incluyendo su " +
    ...
    inputSchema: z.object({
        nombre: z
            .string()
            .describe("Nombre de la nota, como lo devuelve listar_temas. Ejemplo: 'Modelo 4+1 vistas'."),
    }),
    annotations: { readOnlyHint: true },
}, async ({ nombre }) => ejecutar("leer_nota", () => {
...
server.registerTool("buscar", {
    ...
    description: "Busca un texto en las notas de la materia, en el glosario y en las referencias de " +
    ...
    annotations: { readOnlyHint: true },
}, async ({ consulta }) => ejecutar("buscar", () => {
...
// RF-04 — glosario
// ---------------------------------------------------------------------------
server.registerTool("glosario", {
    title: "Consultar el glosario",
    description: "Devuelve la definicion breve de un termino de la materia y la nota donde se " +
    ...
    annotations: { readOnlyHint: true },
}, async ({ termino }) => ejecutar("glosario", () => {
...
server.registerTool("listar_diagramas", {
    ...
    description: "Inventario de todos los diagramas de la boveda: los archivos .excalidraw y .svg " +
    ...
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => ejecutar("listar_diagramas", () => {
    const diagramas = listarDiagramas();
    if (diagramas.length === 0)
        return "No hay diagramas en la boveda.";
    ...
    if (bloques.length > 0) {
        ...
        for (const d of bloques) {
            ...
            if (d.primeraLinea)
                lineas.push(`    empieza con: ${d.primeraLinea}`);
        }
        lineas.push("");
    }
...
server.registerTool("obtener_diagrama", {
    ...
    description: "Devuelve la FUENTE CRUDA de un diagrama: el bloque mermaid tal cual esta escrito " +
    ...
    inputSchema: z.object({
        nombre: z
            .string()
            .describe("Id del diagrama de listar_diagramas, nombre de archivo, o nombre de una nota. " +
            "Ejemplo: 'Modelo 4+1 vistas#mermaid-1'."),
    }),
    annotations: { readOnlyHint: true },
}, async ({ nombre }) => ejecutar("obtener_diagrama", () => {
...
// ---------------------------------------------------------------------------
server.registerTool("obtener_flashcards", {
    title: "Obtener flashcards de un tema",
    description: "Devuelve las tarjetas de repaso de un tema en pares pregunta/respuesta, desde " +
    ...
    inputSchema: z.object({
        ...
        cantidad: z
            ...
            .positive()
            .optional()
            .describe("Cuantas tarjetas devolver, tomadas desde el principio. Si se omite, devuelve todas."),
    }),
    annotations: { readOnlyHint: true },
}, async ({ tema, cantidad }) => ejecutar("obtener_flashcards", () => {
...
server.registerTool("registrar_resultado", {
    ...
    description: "Agrega una linea con la fecha de hoy al registro 05-Quizzes/progreso.md. " +
...
}, async ({ tema, puntaje, comentarios }) => ejecutar("registrar_resultado", () => {
...
// RF-09 — progreso
// ---------------------------------------------------------------------------
server.registerTool("progreso", {
    title: "Consultar el progreso de estudio",
    description: "Resume el historial de quizzes: temas ya evaluados con su ultimo puntaje, mejor " +
    ...
    if (p.totalEvaluaciones === 0) {
        ...
        lineas.push(`${p.totalEvaluaciones} evaluaciones registradas. Promedio general: ${p.promedioGeneral}/100.`, "", "## Temas evaluados");
        for (const t of p.temasEvaluados) {
            lineas.push(`- ${t.tema}: ultimo ${t.ultimoPuntaje}/100 (${t.ultimaFecha}) | ` +
    ...
    if (p.temasPendientes.length === 0) {
        ...
        for (const tema of p.temasPendientes)
            lineas.push(`- ${tema}`);
    }
    return lineas.join("\n");
}));
...
// RF-10 — referencia
// ---------------------------------------------------------------------------
server.registerTool("referencia", {
    title: "Consultar la referencia de una herramienta",
    description: "Devuelve el manual condensado de las herramientas de dibujo del ecosistema y el puente " +
    ...
    inputSchema: z.object({
        herramienta: z
            .string()
            .optional()
            .describe("Nombre de la referencia: 'staruml', 'excalidraw' o 'de la teoria al diagrama'. " +
            "Si se omite, devuelve la lista de referencias disponibles con un resumen de cada una."),
    }),
    annotations: { readOnlyHint: true },
}, async ({ herramienta }) => ejecutar("referencia", () => {
