 ...
 */
export function consultarGlosario(termino) {
    ...
    if (porTermino.length > 0)
        return porTermino;
    const porDefinicion = todas.filter((e) => clave(e.definicion).includes(buscado));
    if (porDefinicion.length > 0)
        return porDefinicion;
    throw new ErrorHerramienta(`El termino "${termino}" no esta en el glosario.`, sugerir(termino, todas.map((e) => e.termino)));
}
//# sourceMappingURL=glosario.js.map