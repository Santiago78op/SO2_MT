 ...
 */
import { ErrorHerramienta, clave, leerTexto, listarArchivos, nombreDeNota, paraMostrar, parsearFrontmatter, rutaRelativa, } from "./boveda.js";
 ...
 */
function primerParrafo(cuerpo, maximo = 220) {
 ...
 */
export function obtenerReferencia(herramienta) {
    const disponibles = listarReferencias();
    if (disponibles.length === 0) {
        throw new ErrorHerramienta(`No hay referencias en ${CARPETA_REFERENCIAS}/. Todavia no se escribieron.`);
    }
    ...
    elegida ??= disponibles.find((r) => clave(r.tema).includes(clave(herramienta)) || clave(r.resumen).includes(clave(herramienta)));
    if (!elegida) {
        throw new ErrorHerramienta(`No tengo una referencia para "${herramienta}".`, disponibles.map((r) => `${r.nombre}  (${r.tema})`));
    }
    return {
...
//# sourceMappingURL=referencias.js.map