...
async function main() {
    ...
    if (idEncontrado) {
        ...
        mostrar(await client.callTool({ name: "obtener_diagrama", arguments: { nombre: idEncontrado } }), 900);
    }
    ...
    console.log((desde >= 0 ? textoRef.slice(desde, desde + 800) : textoRef.slice(0, 800))
        .split("\n")
        .map((l) => `  ${l}`)
        .join("\n"));
    ...
    if (process.env["PERMITIR_ESCRITURA"] === "1") {
        ...
        console.log("  Omitida porque ESCRIBE en la boveda. Para ejecutarla, apunta VAULT_PATH a una\n" +
