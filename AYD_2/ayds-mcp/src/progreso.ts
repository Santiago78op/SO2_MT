 ...
 */
function plantillaInicial(): string {
  return [
    "---",
    "tema: Progreso",
 ...
 */
function escaparParaTabla(texto: string): string {
  return texto
    .replace(/\r?\n/g, " ")
    .replace(/\|/g, "\\|")
    .trim();
...
export function registrarResultado(
  ...
  puntaje: number,
  comentarios?: string,
): { linea: string; archivo: string; creado: boolean } {
  ...
  if (!Number.isFinite(puntaje)) {
    throw new ErrorHerramienta(
      `El puntaje "${puntaje}" no es un numero. Se espera un valor de 0 a 100.`,
    );
  }
  if (puntaje < 0 || puntaje > 100) {
    throw new ErrorHerramienta(
      `El puntaje ${puntaje} esta fuera de rango. Se espera un valor de 0 a 100 ` +
  ...
  const linea = `| ${fechaDeHoy()} | ${temaLimpio} | ${redondeado} | ${
    comentarios ? escaparParaTabla(comentarios) : "—"
  } |`;
...
export function leerRegistros(): RegistroProgreso[] {
  ...
  const registros: RegistroProgreso[] = [];
...
export interface Progreso {
  ...
  temasEvaluados: ResumenTema[];
  /** Temas que tienen notas en la boveda pero nunca fueron evaluados. */
 ...
 */
export function obtenerProgreso(): Progreso {
  ...
  const porTema = new Map<string, { nombre: string; items: RegistroProgreso[] }>();
  ...
  const temasEvaluados: ResumenTema[] = [...porTema.values()].map(({ nombre, items }) => {
