...
function verificar(descripcion: string, condicion: boolean, detalle = ""): void {
  if (condicion) {
    pasadas++;
...
function verificarRechazo(descripcion: string, fn: () => unknown): void {
  try {
    fn();
...
writeFileSync(
  join(temporal, "01-Notas", "Nota simple.md"),
  ["---", "tema: Tema de prueba", "fecha: 2026-08-19", "---", "", "Contenido.", ""].join("\n"),
  "utf8",
);
...
writeFileSync(
  join(temporal, "01-Notas", nombreNFD),
  ["---", "tema: Tema con acentos", "fecha: 2026-08-19", "---", "", "Cuerpo.", ""].join("\n"),
  "utf8",
);

writeFileSync(
  join(temporal, "03-Glosario.md"),
  ["---", "tema: Glosario", "---", "", "## A", "- **Alfa** — primera letra. → [[Nota simple]]", ""].join("\n"),
  "utf8",
);
...
verificar(
  "una ruta normal dentro de la boveda se acepta",
  rutaSegura("01-Notas/Nota simple.md").startsWith(raiz()),
);
...
if (symlinkCreado) {
  verificarRechazo("rechaza un symlink que apunta fuera de la boveda", () =>
    rutaSegura("01-Notas/atajo.txt"),
  );
} else {
...
verificarRechazo("rechaza una carpeta hermana con prefijo parecido", () =>
  rutaSegura(`../${raiz().split(/[\\/]/).pop()}-privado/x.md`),
);
...
verificar(
  "NFC y NFD son cadenas DISTINTAS (por eso hace falta normalizar)",
  enNFC !== enNFD,
  `NFC=${enNFC.length} chars, NFD=${enNFD.length} chars`,
);
...
try {
  ...
  const e = error as ErrorHerramienta;
...
verificar(
  "un comentario con '|' se escapa y la fila mantiene 4 columnas",
  filaPipe.split(/(?<!\\)\|/).filter((c) => c.trim().length > 0).length === 4,
  `fila: ${filaPipe}`,
);
...
verificar(
  "recupera el comentario con pipe sin el escape",
  registros.some((r) => r.comentarios.includes("include | extend")),
);
...
verificar(
  "'Tema de prueba' aparece como evaluado y NO como pendiente",
  p.temasEvaluados.some((t) => clave(t.tema) === clave("Tema de prueba")) &&
    !p.temasPendientes.some((t) => clave(t) === clave("Tema de prueba")),
);
verificar(
  "los pendientes salen de los temas de las notas",
  p.temasPendientes.every((t) => temasEnNotas.some((n) => clave(n) === clave(t))),
  `pendientes: ${p.temasPendientes.join(", ") || "(ninguno)"}`,
);
...
verificar(
  "encuentra por tokens una consulta que no aparece literal",
  porTokens.length > 0 && porTokens.some((c) => clave(c.fragmento).includes(clave("primera letra"))),
  `resultados: ${porTokens.length}`,
);
...
verificar(
  "ninguna coincidencia se repite (ruta:linea unicos)",
  llaves.length === new Set(llaves).size,
  `${llaves.length} resultados, ${new Set(llaves).size} unicos`,
);

// La frase exacta tiene que puntuar mas que un match por tokens sueltos.
const exacta = buscar("primera letra");
verificar(
  "la frase exacta puntua 100 o mas",
  exacta.length > 0 && (exacta[0]?.puntaje ?? 0) >= 100,
  `puntaje del primero: ${exacta[0]?.puntaje}`,
);
