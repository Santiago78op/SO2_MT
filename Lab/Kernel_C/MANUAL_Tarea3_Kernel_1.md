...
| ⛔ | **No avances** hasta que esto dé bien. Hay solo 4 en todo el manual. |
...
| Ruta | Qué es |
|---|---|
| `~/kernel/linux-6.12.69/` | Fuente del kernel — **la mayoría de comandos van acá** |
...
| [1.11](#p11) | Desactivar firmas de módulos | — | 5 min |
...
| [T6](#t6) | Firmas de módulos y Secure Boot |
| [T7](#t7) | Initramfs, GRUB y el arranque |
| [T8](#t8) | Paralelismo y ley de Amdahl |
| [T9](#t9) | **Estructura del informe + checklist de 30 capturas** |

### 🔥 [PARTE 3 — ERRORES](#parte-3)

| § | Tema |
|---|---|
| [E1](#e1) | Errores de configuración y compilación |
| [E2](#e2) | Errores de arranque |
...
⛔ **Debe imprimir `6.12.69-tunombre-tucarne`.** Si no, arreglalo antes de seguir → [§1.13](#p13)
...
**ESC** al arrancar → **Advanced options** → 📸 captura del menú → elegí **tu** kernel.
...
### Los 4 puntos donde no te podés equivocar
...
### Confirmar que la versión existe
...
> Si `linux-6.12.69.tar.xz` no aparece, usá la 6.12.x más alta que sí esté y **reemplazá el número en todos los comandos siguientes**. Anotá el cambio en el informe.
...
⛔ **Esperás `linux-6.12.69.tar.xz: OK`.** Si dice `FAILED`, la descarga se corrompió: `rm` y bajala de nuevo. **No descomprimas un tarball que falló el checksum** — vas a perder horas persiguiendo errores fantasma.

```bash
time tar -xf linux-6.12.69.tar.xz      # 1-3 min, son ~85,000 archivos
cd linux-6.12.69
```
...
```bash
{
  ...
  echo "Fecha: $(date)"
  echo; echo "--- Checksum ---"
  (cd ~/kernel && grep 'linux-6.12.69.tar.xz$' sha256sums.asc | sha256sum -c -)
...
> El `yes "" |` responde **Enter** (aceptar el default) a cualquier opción nueva. Como Debian 13 trae 6.12 y el fuente es 6.12.69, deberían ser muy pocas o ninguna.
...
<a name="p11"></a>
## 1.11 — Desactivar la verificación de firmas de módulos

> 🎯 **Requisito obligatorio del enunciado.** El README del curso solo cubre la mitad; acá está completo.
...
```bash
# (a) Vaciar las rutas a llaveros que no existen
scripts/config --set-str SYSTEM_TRUSTED_KEYS ""
scripts/config --set-str SYSTEM_REVOCATION_KEYS ""

# (b) Desactivar el mecanismo de firma completo
scripts/config --disable MODULE_SIG
scripts/config --disable MODULE_SIG_ALL
scripts/config --disable MODULE_SIG_FORCE
...
**📸 EVIDENCIA:** dos capturas — el `grep` **antes** (con `debian/canonical-certs.pem`) y **después** (con `is not set` y `""`). Es el par de imágenes que prueba este requisito.
...
| Regla | Por qué |
|---|---|
| Empieza con **guion** `-` | Se concatena directo a `6.12.69` |
...
| 5 | `SYSTEM_TRUSTED_KEYS` | `""` y `MODULE_SIG` en *not set* |
...
|---|---|---|
| `vmlinuz-<ver>` | El kernel comprimido y booteable | 15–25 MB |
| `initrd.img-<ver>` | El initramfs con los módulos de arranque | 30–90 MB |
...
**📸 EVIDENCIA:** `ls -lh /boot/` mostrando **los cuatro archivos con tu nombre en el nombre del archivo**, y `ls -d /lib/modules/<tu-versión>`.
...
⛔ **Si tu versión NO aparece en `grub.cfg`, NO reinicies.** Corré `sudo update-grub` otra vez y buscá en su salida `Found linux image: /boot/vmlinuz-<tu-versión>`.
...
1. 📸 **Captura del menú principal** (en macOS: `Cmd+Shift+4`).
2. Andá a **"Advanced options for Debian GNU/Linux"** → Enter.
3. 📸 **Captura de la lista de kernels** — acá se ven **ambos**: el de Debian y **el tuyo con tu nombre**. Una de las mejores capturas del informe.
4. Seleccioná **tu kernel** (`6.12.69-tunombre-tucarne`) → Enter.

> En x86 con BIOS entrarías al menú con SHIFT. En **UEFI/arm64 se usa ESC**.
...
```bash
{
  ...
  echo; echo "=== Firmas ==="; grep -E 'CONFIG_MODULE_SIG' /boot/config-$(uname -r)
  echo; echo "=== Raiz ==="; findmnt /
...
|---|---|---|
| `vmlinuz-<ver>` | El kernel comprimido y booteable | `/boot/` |
...
**Sobre `CONFIG_LOCALVERSION`:** Debian lo usa para embeber el nombre de su *flavour* (`-arm64`). Como se copia su `.config`, si no se vacía el string final sale `6.12.69-tunombre-arm64`. Y `CONFIG_LOCALVERSION_AUTO` invoca `scripts/setlocalversion`, que agrega un `+` cuando detecta un árbol git modificado.
...
El enunciado pide desactivar **(b)**; el README del curso solo cubre **(a)**. Hay que hacer las dos cosas.
...
Por eso `make install` corre `update-initramfs`, y por eso `/boot` necesita espacio: cada kernel instalado trae su propio initramfs de 30–90 MB.
...
**GRUB** (*GRand Unified Bootloader*) es un bootloader multi-etapa. En arm64 el firmware UEFI carga `grubaa64.efi` desde la partición EFI, GRUB lee `/boot/grub/grub.cfg`, presenta el menú, y para la opción elegida ejecuta dos comandos: `linux /boot/vmlinuz-<ver> <parámetros>` e `initrd /boot/initrd.img-<ver>`. Carga ambos en RAM y transfiere el control al kernel.

**El kernel nuevo NO reemplaza al de Debian: convive con él.** `update-grub` escanea `/boot` y genera una entrada por cada `vmlinuz-*` que encuentra. Esa es la red de seguridad: si el kernel nuevo no arranca, se reinicia, se entra a *Advanced options* y se elige el de Debian. **Nunca borres el kernel original.**
...
| 15 | `grep MODULE_SIG` **antes** | [1.11](#p11) | Firmas |
| 16 | `grep MODULE_SIG` **después** | [1.11](#p11) | Firmas |
...
| 23 | `ls -lh /boot/` con los 4 archivos | [1.15](#p15) | Evidencias |
...
Compilás el kernel `6.12.69` **nativamente para ARM64** en una VM **Debian 13 arm64** sobre VMware Fusion: copiás la config de Debian, la reducís con `localmodconfig`, desactivás la verificación de firmas, escribís tu nombre y carné en el `EXTRAVERSION` del Makefile, compilás con `fakeroot make -j$(nproc)`, instalás con `modules_install` + `install`, booteás desde GRUB y lo probás con `uname -r` — verificando en **cada** paso antes de avanzar, y guardando la evidencia de cada verificación para el informe.
